...
import {
  ...
  createBrickDisplacementMap,
  createRoofTileTexture,
  createRoofRoughnessMap,
  createRoofNormalMap,
  createRoofDisplacementMap
} from './textures';
...
export function createBuilding(options: BuildingOptions = {}): THREE.Group {
  ...
  const brickDisplacement = createBrickDisplacementMap();
  brickTexture.repeat.set(4, 2);
  brickRoughness.repeat.set(4, 2);
  brickNormal.repeat.set(4, 2);
  brickDisplacement.repeat.set(4, 2);
  ...
  const roofNormal = createRoofNormalMap();
  const roofDisplacement = createRoofDisplacementMap();
  roofTileTexture.repeat.set(3, 4);
  roofRoughness.repeat.set(3, 4);
  roofNormal.repeat.set(3, 4);
  roofDisplacement.repeat.set(3, 4);
  ...
  const roofMaterial = new THREE.MeshStandardMaterial({
    ...
    displacementMap: roofDisplacement,
  ...
  const frontWall = new THREE.Mesh(
    new THREE.BoxGeometry(baseWidth, wallHeight, wallThickness, 16, 8, 1),
    wallMaterial
  );
  ...
  const leftWall = new THREE.Mesh(
    new THREE.BoxGeometry(wallThickness, wallHeight, baseDepth, 1, 8, 12),
    wallMaterial
  );
  ...
  const roofHeight = 2;
  const roofGeometry = new THREE.ConeGeometry(baseWidth * 0.75, roofHeight, 4, 16, true);
  ...
  const muntinH = new THREE.Mesh(
    new THREE.BoxGeometry(largeGlassWidth, 0.06, frameDepth * 0.8),
    windowFrameMaterial
  );
  muntinH.position.set(0, 0, -baseDepth / 2 + wallThickness / 2 + 0.03);
  building.add(muntinH);

  const muntinV = new THREE.Mesh(
    new THREE.BoxGeometry(0.06, largeGlassHeight, frameDepth * 0.8),
    windowFrameMaterial
  );
  muntinV.position.set(0, 0, -baseDepth / 2 + wallThickness / 2 + 0.03);
  building.add(muntinV);
  ...
  doorKnob.position.set(-baseWidth / 4 + doorWidth / 2 - 0.15, doorHeight / 2, -baseDepth / 2 + wallThickness / 2 + 0.18);
  building.add(doorKnob);
