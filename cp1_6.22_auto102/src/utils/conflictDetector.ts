...
export const conflictRules: ConflictRule[] = [
  ...
  { id: 2, keywordsA: ['加班', '延长工作时间', '996'], keywordsB: ['准时下班', '按时下班', '不加班', '工作生活平衡'] },
  ...
  { id: 10, keywordsA: ['开源', '免费'], keywordsB: ['收费', '商业化', '付费模式', '闭源'] },
  { id: 11, keywordsA: ['问题', '困难', '挑战', '障碍'], keywordsB: ['方案', '解决', '对策', '方法'] },
  { id: 12, keywordsA: ['风险', '隐患', '危机'], keywordsB: ['机会', '机遇', '优势'] },
];

export function detectConflicts(nodes: GraphNode[], links: GraphLink[]): Conflict[] {
  ...
  for (const link of links) {
    ...
    const sourceText = `${sourceNode.title} ${sourceNode.description} ${sourceNode.tags.join(' ')}`;
    const targetText = `${targetNode.title} ${targetNode.description} ${targetNode.tags.join(' ')}`;
...
export function getNodeTagColor(title: string, tags: string[]): string {
  const text = `${title} ${tags.join(' ')}`;
  if (text.includes('问题') || text.includes('困难') || text.includes('挑战') || text.includes('障碍')) {
    return '#e53e3e';
  }
  if (text.includes('方案') || text.includes('解决') || text.includes('对策') || text.includes('方法')) {
    return '#38a169';
  }
  if (text.includes('风险') || text.includes('隐患') || text.includes('危机')) {
    return '#dd6b20';
  }
  if (text.includes('机会') || text.includes('机遇') || text.includes('优势')) {
    return '#3182ce';
  }
  return '#718096';
}
