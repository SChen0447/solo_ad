...
router.get('/bookmarks/tags', (_req: Request, res: Response) => {
  const allTags = [...new Set(bookmarks.flatMap(b => b.tags))]
  res.json({ success: true, data: allTags })
})

router.post('/bookmarks/fetch-url', (req: Request, res: Response) => {
  const { url } = req.body
  if (!url) {
...
router.post('/bookmarks', (req: Request, res: Response) => {
  ...
  if (!URL_PATTERN.test(url)) {
    res.status(400).json({ success: false, error: 'Invalid URL format. Must start with http:// or https:// and contain a valid domain' })
    return
  }

  if (tags && (!Array.isArray(tags) || tags.length > 5)) {
    res.status(400).json({ success: false, error: 'Tags must be an array with maximum 5 items' })
    return
  }
...
router.post('/bookmarks/batch-delete', (req: Request, res: Response) => {
  const { ids } = req.body
...
router.post('/bookmarks/batch-tag', (req: Request, res: Response) => {
  const { ids, tags } = req.body
  if (!ids || !Array.isArray(ids) || !tags || !Array.isArray(tags)) {
    res.status(400).json({ success: false, error: 'ids and tags arrays are required' })
    return
  }
...
router.post('/bookmarks/batch-move', (req: Request, res: Response) => {
...
router.post('/bookmarks/:id/share', (req: Request, res: Response) => {
  const { id } = req.params
  const bookmark = bookmarks.find(b => b.id === id)
