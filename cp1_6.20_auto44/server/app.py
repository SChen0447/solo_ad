...
ai_tags = ['AI精选', '深度分析', '热点追踪', '独家报道', '专家解读']
...
def generate_article(article_id, category):
    title = titles_pool[category][(article_id - 1) % 10]
    ai_tag = random.choice(ai_tags)
    
    num_paragraphs = random.randint(6, 10)
    selected_paragraphs = random.sample(paragraphs_pool, num_paragraphs)
    content = '\n\n'.join(selected_paragraphs)
    
    num_sentences = random.randint(2, 4)
    summary_sentences = random.sample(summary_sentences_pool, num_sentences)
    summary = ' '.join(summary_sentences)
    
    highlight_indices = sorted(random.sample(range(num_paragraphs), min(num_sentences, num_paragraphs)))
...
def generate_news_batch(page, per_page=10, category=None):
    news_list = []
    ...
    for i in range(per_page):
        ...
        news_list.append(generate_article(article_id, cat))
...
def get_news():
    ...
    news_list = generate_news_batch(page, per_page, category)
...
def get_news_detail(news_id):
    category = categories[(news_id - 1) % len(categories)]
    article = generate_article(news_id, category)
