...
import { ConceptNode, Connection, CANVAS_CONFIG } from '../frontend/types';
...
export const Whiteboard: React.FC = () => {
    ...
    const animate = () => {
      if (zoomAnimationRef.current.active) {
        ...
        const currentZoom =
          ...
          (zoomAnimationRef.current.targetZoom - zoomAnimationRef.current.startZoom) *
            easedProgress;
  ...
  const handleMouseDown = useCallback(
    (e: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      if (
        ...
        hitNode &&
        connectingFromNodeId &&
        hitNode.id !== connectingFromNodeId
      ) {
      ...
      if (isDraggingRef.current && dragNodeIdRef.current) {
        ...
        if (node) {
          ...
          if (guides.length > 0) {
            ...
            const snapNode = guides[0].targetNodeId ? nodesMap.get(guides[0].targetNodeId) : null;
            if (snapNode) {
  ...
  const handleDoubleClick = useCallback(
    ...
    [getCanvasCoords, hitTestNode, isConnecting, currentStyle, addNode, selectNode]
  );
  ...
  }, [selectedNodeId, selectedConnectionId, isConnecting, cancelConnecting, deleteNode]);
